...
func getSLAs(c *ExporterClient) (*[]SLAStats, error) {
	...
	for _, customSLA := range c.slaCustomFields {
		...
		if c.teamCustomField != "" {
			max = 500
			expression = fmt.Sprintf("issues.map(issue => ({status: issue.status.name, teamName: issue.%s ? issue.%s.value : 'Unassigned', slaName: issue.%s.name, slaBreached: issue.%s.completedCycles[0] ? issue.%s.completedCycles[0].breached : true , project: issue.project.key })).reduce((result, issue) => result.set(issue.project + ':' + issue.teamName + ':' + issue.slaName,(result[issue.teamName] || {}).set(issue.slaBreached == true ? 'BREACHED' : 'NOT_BREACHED',((result[issue.project + ':' + issue.teamName + ':' + issue.slaName] || {})[issue.slaBreached == true ? 'BREACHED' : 'NOT_BREACHED'] || 0) + 1)),new Map())", c.teamCustomField, c.teamCustomField, customSLA, customSLA, customSLA)
		} else {
			max = 600
